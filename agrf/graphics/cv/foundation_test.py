...
def test_make_foundation_subimage_basic_left_side():
    ...
    out = make_foundation_subimage(base, scale=4, part=0, cut_inside=False, zshift=0, solid=True)
...
def test_make_foundation_passthrough_single_input():
    base = _full_alpha_image()
    # Only solid provided
    expected_solid = make_foundation_subimage(base.copy(), scale=4, part=0, cut_inside=False, zshift=0, solid=True)
    out_solid = make_foundation(base, None, scale=4, part=0, cut_inside=False, zshift=0)
    assert np.array_equal(out_solid.alpha, expected_solid.alpha)

    # Only ground provided
    expected_ground = make_foundation_subimage(base.copy(), scale=4, part=0, cut_inside=False, zshift=0, solid=False)
    out_ground = make_foundation(None, base, scale=4, part=0, cut_inside=False, zshift=0)
...
def test_make_foundation_subimage_zshift_reduces_coverage_for_part0():
    ...
    base = _full_alpha_image()
    base.yofs = 120
    out0 = make_foundation_subimage(base, scale=4, part=0, cut_inside=False, zshift=0, solid=True)
    out1 = make_foundation_subimage(base, scale=4, part=0, cut_inside=False, zshift=2, solid=True)
